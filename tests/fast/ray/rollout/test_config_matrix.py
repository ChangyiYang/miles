...
class TestResolveDefaults:
    def test_resolve_inherits_num_gpus_per_engine_from_args(self, tmp_path):
        """A group without num_gpus_per_engine falls back to the args-level value."""
        cfg = _resolve_yaml(
            tmp_path,
            "sglang:\n  - name: actor\n    server_groups:\n      - worker_type: regular\n        num_gpus: 8\n",
            rollout_num_gpus=8,
    ...
    def test_resolve_inherits_model_path_into_overrides(self, tmp_path):
        """A group without a model_path override gets the checkpoint path."""
        cfg = _resolve_yaml(
            tmp_path,
            "sglang:\n  - name: actor\n    server_groups:\n      - worker_type: regular\n        num_gpus: 8\n",
            rollout_num_gpus=8,
            hf_checkpoint="/path/actor",
        )
        assert cfg.models[0].server_groups[0].overrides["model_path"] == "/path/actor"

    def test_resolve_auto_infers_update_weights_false_for_diff_path(self, tmp_path):
        """A model on a different checkpoint defaults to frozen weights."""
        cfg = _resolve_yaml(
            tmp_path,
            "sglang:\n"
            "  - name: ref\n"
    ...
    def test_resolve_auto_infers_update_weights_true_for_same_path(self, tmp_path):
        """A model on the training checkpoint defaults to receiving weight updates."""
        cfg = _resolve_yaml(
            tmp_path,
            "sglang:\n"
            "  - name: actor\n"
    ...
    def test_resolve_explicit_update_weights_not_overridden(self, tmp_path):
        """An explicit update_weights survives resolve even when the paths match."""
        cfg = _resolve_yaml(
            tmp_path,
            "sglang:\n"
            "  - name: ref\n"
