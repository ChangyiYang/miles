...
def compute_sglang_router_args(
    ...
    return _fill_unset_from_cli_defaults(router_args)
...
def _fill_unset_from_cli_defaults(router_args: RouterArgs) -> RouterArgs:
    ...
    for field in dataclasses.fields(router_args):
        if getattr(router_args, field.name) is None:
            setattr(router_args, field.name, getattr(cli_defaults, field.name))
    return router_args


def router_args_to_argv(router_args: RouterArgs) -> list[str]:
    return render_cli_argv(router_args, make_parser=_make_cli_parser, from_parsed=RouterArgs.from_cli_args)
