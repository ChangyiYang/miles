...
# Prepare script for Qwen3-32B (dense): download model/datasets and convert checkpoint.
#
# Run this BEFORE run-qwen3-32B-2node-profile.sh.
#
# Usage:
#   bash prepare-qwen3-32B.sh
...
# must match --actor-num-gpus-per-node in the run script: the torch_dist
# checkpoint is sharded for this layout
GPUS_PER_NODE=8
